...
if forecast_points.empty:
    ...
    quad_labels = pd.DataFrame([
        {"x": x0 * 0.20, "y": y_max * 0.70 if y_max > 0 else 5,  "t": "Emerging Deteriorating"},
        {"x": x0 + (x_max - x0) * 0.55, "y": y_max * 0.70 if y_max > 0 else 5, "t": "Accelerating Deteriorating"},
        {"x": x0 * 0.20, "y": y_min * 0.70 if y_min < 0 else -5, "t": "Emerging Improving"},
        {"x": x0 + (x_max - x0) * 0.55, "y": y_min * 0.70 if y_min < 0 else -5, "t": "Accelerating Improving"},
    ])
